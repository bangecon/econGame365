...
##' @param externality is the external cost of pollution for each unit of output firms produce (default is 3).
##' @param tax is the amount of the Pigouvian tax on the externality in step 3 (default is 3).
##' @param practice is a logical indicating whether step 1 is practice (default is \code{TRUE}).
##'
...
pollutionGame <-
  function(filename,
    ...
    # Set up the Excel path, read responses, and initialize output objects.
    ...
    results3$FirmCost <-
      I(results$Cleanup3 == 0) * (tax * 2) +
      I(results$Cleanup3 == 1) * (results$Cost1 + tax) +
      I(results$Cleanup3 == 2) * (results$Cost2)
    results3$SocialCost <-
    ...
    writepath <- paste0(drive, user, team, subdir, "Pollution Game Grades.csv")
    write.csv(grades$grades, file = writepath, row.names = FALSE)
    out <- list(
      type = "pollutionGame",
      results = results,
