...
plot.econGame <- function(econGame,
  ...
  if (econGame$type %in% c('cropchoiceGame', 'pestcontrolGame', 'staghuntGame', 'codeGame')) {
    out <- ggplot() +
  ...
  if (econGame$type == 'ultimatumGame') {
    out <- ggplot() +
      geom_histogram(aes(Offer, fill = Outcome),
                     data = econGame$results,
                     position = 'stack',
                     na.rm = TRUE,
                     binwidth = 0.5,
                     alpha = 0.75) +
      scale_fill_manual(values = c('darkorange', 'darkorange3'), breaks = c("Accept", "Reject")) +
      lims(x = c(-1, max(econGame$results$Offer) + 1))
  }
  if (econGame$type == 'nashdemandGame') {
    out <- ggplot() +
      geom_histogram(aes(Demand, fill = Outcome),
                     data = econGame$results,
                     position = 'stack',
                     na.rm = TRUE,
                     binwidth = 0.5,
                     alpha = 0.75) +
      scale_fill_manual(values = c('darkorange', 'darkorange3'), breaks = c("Accept", "Reject")) +
      lims(x = c(-1, max(econGame$results$Demand) + 1))
  }
  if (econGame$type %in% c('cournotGame', 'cattleGame')) {
    out <- ggplot() +
      geom_bar(aes(x = Outcome, y = after_stat(prop), group = 1),
  ...
  if (econGame$type == 'bertrandGame') {
    out <- ggplot(econGame$results) +
      geom_bar(aes(x = Price, y = after_stat(prop)), fill = 'darkorange',
               alpha = 0.3, ...) +
      geom_bar(aes(x = Market.Price, y = after_stat(prop)), fill = 'blue',
               alpha = 0.3, ...)
  }
